...
const SettingsPage: React.FC = () => {
  ...
  const handleExport = () => {
    ...
    toast.success(translate("Данные экспортированы! 📦"));
  };
  ...
  const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        toast.success(translate("Данные импортированы! 🔄"));
        setTimeout(() => window.location.reload(), 1000);
      } catch {
        toast.error(translate("Ошибка при импорте"));
      }
    };
  ...
  const handleClearData = () => {
    if (confirm(translate("Вы уверены, что хотите удалить все данные?"))) {
      localStorage.clear();
      toast.success(translate("Все данные удалены"));
      setTimeout(() => window.location.reload(), 1000);
    }
  };
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between p-4 bg-purple-50 dark:bg-purple-900/20 rounded-2xl">
            <div>
              <h3 className="font-semibold text-gray-800 dark:text-white">
                {translate("Тема оформления")}
              </h3>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {translate("Выберите светлую или тёмную тему")}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between p-4 bg-purple-50 dark:bg-purple-900/20 rounded-2xl">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {translate("Выберите язык приложения")}
              </p>
            </div>
            <div className="flex gap-2">
              <button
          ...
          <div className="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-2xl text-center">
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Life Dashboard v1.0.0 • {translate("Сделано с 💜")}
            </p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
              {translate("Данные хранятся локально в вашем браузере")}
            </p>
          </div>
        </div>
      </motion.div>
